...
function initializeWebGLViewer() {
    document.querySelectorAll('.glb-viewer').forEach(container => {
        ...
        const gui = new lil.GUI(); // Initialize lil-gui
...
async function formatUniformsAsync(uniformData, materialPath, renderer) {
    ...
    for (const key of Object.keys(data)) {
        const { type, value, wrap, min, max, step } = data[key]; // Extract min, max, step
        ...
        if (type === 'sampler2D') {
            ...
            // Pass through raw values (vec3, float, etc.) along with GUI config
            formatted[key] = { value };
            if (min !== undefined) formatted[key].min = min;
            if (max !== undefined) formatted[key].max = max;
            if (step !== undefined) formatted[key].step = step;
        }
    }
...
function addMaterialGUI(gui, material, uniforms) {
    ...
    for (const key in uniforms) {
        ...
        if (uniform.value instanceof THREE.Color) {
            ...
            // Add sliders for vec3 or vec2 uniforms with configurable ranges
            const min = uniform.min !== undefined ? uniform.min : -10;
            const max = uniform.max !== undefined ? uniform.max : 10;
            const step = uniform.step !== undefined ? uniform.step : 0.1;
            uniform.value.forEach((_, i) => {
                folder.add(uniform.value, i, min, max, step).name(`${key}[${i}]`);
            });
        } else if (typeof uniform.value === 'number') {
            // Add slider for float uniforms with configurable ranges
            const min = uniform.min !== undefined ? uniform.min : 0.0;
            const max = uniform.max !== undefined ? uniform.max : 10.0;
            const step = uniform.step !== undefined ? uniform.step : 0.01;
            folder.add(uniform, 'value', min, max, step).name(key);
